class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        for bucket in buckets[::-1]: #10001
            for num in bucket: #1 = 10,000 = constant
                if len(resultList) == k:
                    break
                else:
...
#Final solution
#Do as above and once hashMap full
#Make constant = 10001 (as specified by constraints) buckets, each bucket index in a larger list
#of buckets is the frequency at which nums occurs in the input list, then the value at that index is 
#the number that 
